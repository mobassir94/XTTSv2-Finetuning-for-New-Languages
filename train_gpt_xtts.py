...
def train_gpt(metadatas, num_epochs, batch_size, grad_acumm, output_path, max_audio_length, max_text_length, lr, weight_decay, save_step):
    ...
    DVAE_CHECKPOINT_LINK = "XTTS-v2/main/dvae.pth"
    MEL_NORM_LINK = "XTTS-v2/main/mel_stats.pth"
    ...
    # Print vocabularies (full vocab for inspection)
    vocab = None
    if hasattr(tokenizer, 'vocab'):
        ...
        print("[!] Could not find vocab attribute in tokenizer.")

    if vocab:
        print("3. Vocabulary list (full):")
    ...
    unk_tokens = []
    if vocab:
        unk_keys = [k for k in vocab.keys() if 'unk' in k.lower() or '<unk>' in k.lower() or '[unk]' in k.lower()]
        if unk_keys:
