...
def train_gpt(metadatas, num_epochs, batch_size, grad_acumm, output_path, max_audio_length, max_text_length, lr, weight_decay, save_step):
    ...
    print(f"1. Tokenizer path: {config.model_args.tokenizer_file}")
    try:
        tok_len = len(tokenizer)
    except Exception as e:
        tok_len = None
        print(f"[!] Could not get tokenizer length: {e}")
    print(f"2. Tokenizer length: {tok_len}")

    # Print vocabularies (compatible with HF and standard)
    vocab_items = None
    try:
        ...
        print(f"[!] Error extracting vocab: {e}")

    if vocab_items:
        print("3. Vocabulary list (showing first 50):")
        for i, (token, idx) in enumerate(vocab_items):
            print(f"   {i+1}. '{token}': {idx}")
            if i >= 49:
                break
    else:
        print("[!] Could not iterate vocabulary entries.")
    print("================ END TOKENIZER INFO ================\n")

    # Check for unknown tokens in the vocabulary
    unk_token_found = False
    unk_tokens = []
    if vocab_items:
        unk_keys = [k for k, v in vocab_items if 'unk' in str(k).lower() or '<unk>' in str(k).lower() or '[unk]' in str(k).lower()]
        if unk_keys:
            unk_token_found = True
            unk_tokens = unk_keys
    if unk_token_found:
        print(f"[!] Unknown token(s) found in vocabulary: {unk_tokens}")
    else:
        print("[✓] No unknown token found in vocabulary.")
